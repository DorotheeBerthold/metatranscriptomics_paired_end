...
abundances_TX <- read.csv("results/abundances_TX.csv", row.names = 1)
# Remove Muribaculum & Lactobacillus columns
abundances_TX <- abundances_TX %>%
  #filter(mouse_number != "E1") %>% 
  select(-c(Bifidobacterium.animalis.YL2, Lactobacillus.reuteri.I49, Acutalibacter.muris.KB18, Enterococcus.faecalis.KB1,mouse_number, location))
genes_wide <- read.csv("results/gene_counts_wide_metadata.csv", row.names = 1)
genes_wide <- genes_wide %>%
  #filter(mouse != "E1") %>% 
  select(-c(mouse, location))
genes_t <- as.data.frame(t(genes_wide))

# remove genes from YL27 & I49
genes_t <- genes_t %>% 
  filter(!grepl("YL2", rownames(genes_t)),
         !grepl("I49", rownames(genes_t)),
        !grepl("KB18", rownames(genes_t)),
        !grepl("KB1", rownames(genes_t))) 

genes_t$locus_tag <- rownames(genes_t)

# Read in functional locus tags to filter out pseudogenes & tRNAs before normalisation
locus_tags <- read.csv("OMM_files/functional_locus_tags.csv", header = F)

# keep only those locus tags in gene_t that are identical to those in the functional locus tags list
genes_t <- genes_t %>%
  filter(locus_tag %in% locus_tags$V1)
...
cond.vec <- factor(c(rep("dist",3), rep("prox",3)))

# Factor so that "UP" is higher in proximal condition
cond.vec <- factor(cond.vec, levels = c("dist", "prox"))
...
topDE <- head(order(res$padj), 100)
mat <- vsd.counts[topDE, ]
mat <- pheatmap(mat, show_rownames = F, show_colnames = T)
...
# write csv
write.csv( res ,'results/DB104_deseq2_results.csv' )
write.csv(as.data.frame(res.lfc), "results/DB104_deseq2_results_shrunken.csv")
